  ...
  const webScraper = async () => {
    try {
      ...
      while (true) {
        ...
        // Check and click Next button only when it is visible and interactable
        const hasNext = await page.evaluate(async () => {
          const wait = (ms) => new Promise((r) => setTimeout(r, ms));
          const btns = Array.from(document.querySelectorAll("button.artdeco-button"));
